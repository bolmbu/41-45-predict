...
IMG_SIZE = 28             # original canvas size
MODEL_INPUT = 96          # MobileNetV2 recommended minimum
TEST_SIZE = 0.2
RANDOM_STATE = 42
EPOCHS = 45
BATCH_SIZE = 32
...
# ------------------------------------
# Build MobileNetV2 model (Best Accuracy)
# ------------------------------------
def build_model(num_classes):

    base = tf.keras.applications.MobileNetV2(
        input_shape=(MODEL_INPUT, MODEL_INPUT, 3),
        include_top=False,
        weights=None    # train from scratch (ดีที่สุดสำหรับลายมือไทย)
    )

    model = models.Sequential([
        layers.Resizing(MODEL_INPUT, MODEL_INPUT),
        layers.Conv2D(3, (1,1)),  # convert 1‑channel → RGB for MobileNetV2
        base,
    ...
    model.compile(
        optimizer=tf.keras.optimizers.AdamW(learning_rate=0.0008, weight_decay=1e-4),
...
def main():
    ...
    # Strong Augmentation (recommended for handwriting)
